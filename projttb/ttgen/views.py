                ...
                if j >= i:
                    if (classes[i].meeting_time == classes[j].meeting_time) and (classes[i].section_id != classes[j].section_id) and (classes[i].section == classes[j].section):
                        ...
                        if classes[i].meeting_time.day == classes[j].meeting_time.day:
                            if classes[i].meeting_time.time == classes[j].meeting_time.time:
                                self._numberOfConflicts += 1
...
def timetable(request):
    ...
    customSortOrder = { 'M': 0, 'u': 1, 'W': 2, 'h': 3, 'F': 4, 'S': 5 }

    # arrange all the classes in order of their meeting time pid, in custom order, onl the first letter of pid is in cutom order, unles it's tu and th, then it's first 2 ltters, and then sort it by the number after the first letter and if it;s tu ot th then  the number is after 2 nd letter
    schedule.sort(key=lambda x: (customSortOrder[x.meeting_time.pid[(0 if x.meeting_time.pid[0] != 'T' else 1)]], int(x.meeting_time.pid[(1 if x.meeting_time.pid[0] != 'T' else 2)]))) 

    

    # arrange all the classes in order of their meeting time day, the order is M > Tu > W > Th > F > S
